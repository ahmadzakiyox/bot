...
const lyrics_v2_js_1 = require("../types/lyrics-v2.js");
/**
 * Scrape from https://genius.com
 */
async function lyricsv2(query) {
    index_js_1.LyricsArgsSchema.parse(arguments);
    const search = await (0, got_1.default)(`https://genius.com/api/search/multi?per_page=5&q=${encodeURIComponent(query).replace(/%20/, '+')}`).json();
    const json = lyrics_v2_js_1.LyricsV2SearchResponseSchema.parse(search);
    ...
    const [data, html] = await Promise.all([
        (0, got_1.default)(`https://genius.com/api${api_path}`).json(),
        (0, got_1.default)(url).text()
    ]);
    const metadata = lyrics_v2_js_1.LyricsV2MetadataResponseSchema.parse(data);
    ...
    return lyrics_v2_js_1.LyricsV2Schema.parse(result);
}
exports.default = lyricsv2;
